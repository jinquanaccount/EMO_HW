...
from search_method.seacrh_method_smy.my_packing_algorithms import SSBH
...
def my_pack(solution_mode,
            ...
            raw_distance_map):
    residual_box_id_list = copy.deepcopy(raw_box_id_list)
    residual_num_heap_map = copy.deepcopy(raw_num_heap_map)
    res_packed_bin_list = []
    res_total_distance = 0
    res_total_mean_ratio = 0
    total_ratio = 0
    while len(residual_box_id_list) > 0:
        for key in residual_num_heap_map.keys():
            residual_num_heap_map[key].update_by_total_box_list_by_id(residual_box_id_list)
        paths, box_ids = creat_path_and_box_ids_from_solution_mode_and_residual_box_id_list(solution_mode,
                                                                                            residual_num_heap_map,
                                                                                            raw_platform_must_first_map)
        temp_pack = Pack(raw_date_string,
                         box_ids,
                         raw_my_bin_list[0].truck_id,
                         paths,
                         distance_map=raw_distance_map,
                         prob1=1.0,
                         prob2=1.0
                         )
        res, packed_bin_list = temp_pack.run()
        #        if len(packed_bin_list) != 1:  华为代码有问题 单一条路线1辆车会重复
        #           raise Exception("one bin failed")
        packed_bin = packed_bin_list[0]
        total_ratio = total_ratio + packed_bin.ratio
        res_packed_bin_list.append(copy.deepcopy(packed_bin))
        platform_list = res["solutionArray"][0][0]["platformArray"]
        for i in range(len(platform_list) - 1):
            res_total_distance = res_total_distance + raw_distance_map[platform_list[i] + "+" + platform_list[i + 1]]
        res_total_distance = res_total_distance + raw_distance_map[platform_list[-1] + "+end_point"]
        res_total_distance = res_total_distance + raw_distance_map["start_point+" + platform_list[0]]
        for packed_box in packed_bin.packed_box_list:
            residual_box_id_list.remove(packed_box.box_id)
    res_total_mean_ratio = total_ratio / len(res_packed_bin_list)
    return res_packed_bin_list
...
class Pack:
    def __init__(self,
        ...
        # smy增加的属性
        self.distance_map = copy.deepcopy(distance_map)
        self.spu_ids = copy.deepcopy(spu_ids)
...
def my_gen_res(message_str, packed_bin_lists, raw_my_bin_list):
    data = json.loads(message_str)
    res = {"estimateCode": data["estimateCode"], "solutionArray": []}
    truck_list = []
    if packed_bin_lists:
        for packed_bin_list in packed_bin_lists:
            for packed_bin in packed_bin_list:
                truck = {}
                truck["truckTypeId"] = packed_bin.truck_type_id
                truck["truckTypeCode"] = packed_bin.truck_type_code
                ...
                truck["innerWidth"] = packed_bin.width
                truck["innerHeight"] = packed_bin.height
                truck["maxLoad"] = raw_my_bin_list[0].max_Load
                spu_list = []
                for packed_box in packed_bin.packed_box_list:
                    spu = {}
                    packed_box.change_ordinate(packed_bin)
